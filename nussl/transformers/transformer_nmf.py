...
class TransformerNMF(object):
    ...
    Parameters:
        ...
        templates (:obj:`np.array`): Initial state for the templates matrix (also called the 'dictionary')
    ...
    def __init__(self, input_matrix, num_components=50,
        ...
        self.templates = np.random.rand(self.input_matrix.shape[0], num_components) if templates is None else templates
    ...
    def reconstructed_matrix(self):
        ...
        """
        reconstructed_matrix = np.dot(self.templates, self.activation_matrix)
    ...
    def update(self):
        ...
        if self.should_update_template:
            self.templates = self.template_update_func()
    ...
    def plot(self, output_file, matrix_to_db=True, title=None, max_y=None):
        ...
        # below this number, draw lines (`plt.plot()`) for the values instead of plt.imshow()
        plot_line_threshold = 10
        ...
        if matrix_to_db:
            ...
            templates = librosa.logamplitude(np.abs(self.templates) ** 2, ref_power=np.max)
        ...
        # Display the matrix on ax1
        ax1.imshow(matrix)
        ax1.set_ylim(ax1.get_ylim()[::-1])
        # ax1.set_ylim([0, max_y])
        # ax1.set_xlim([min_time, freq])
        ax1.axis('tight')
        ...
        # Display templates on ax2
        if self.num_components > plot_line_threshold:
            ax2.imshow(templates)
        else:
            # bases = np.abs(bases) ** 2
            for a in range(templates.shape[1]):
                ax2.plot((templates[:, a] / 500) + (2 - a), np.arange(0, templates.shape[0]), 'b')
        ...
        if self.num_components > plot_line_threshold:
            ...
            for a in range(activations.shape[0]):
                ax3.plot(np.arange(0, activations.shape[1]), (activations[a, :] / 10) + a, 'b')

        ax3.axis('tight')
        # ax3.get_xaxis().set_visible(False)
        # ax3.get_yaxis().set_visible(False)
        # ax3.grid(which='minor', axis='x', linestyle='-', color='r', linewidth=4)

        # ax3.set_xticks(np.arange(0, time, 1))
        ...
        # Minor ticks
        # ax3.set_xticks(np.arange(-.5, time, 1), minor=True)
        ...
        # Display the matrix on ax4
        ax4.imshow(reconstructed_matrix)
        ...
        ax4.set_title('Reconstructed Matrix')
