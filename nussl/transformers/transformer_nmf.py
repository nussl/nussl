...
class TransformerNMF(object):
    ...
    def __init__(
        ...
        should_update_activation=None,
        should_update_template=None,
        seed=None,
        max_num_iterations=50,
        should_do_epsilon=False,
        stopping_epsilon=1e10,
    ):
        ...
        self.template_dictionary = (
            np.random.rand(self.input_matrix.shape[0], num_components)
            if template_dictionary is None
            else template_dictionary
        )

        # Initialize activation_matrix to random if none provided
        self.activation_matrix = (
            np.random.rand(num_components, self.input_matrix.shape[1])
            if activation_matrix is None
            else activation_matrix
        )

        # Check to make sure we understand the distance measure
        if (
            distance_measure is not None
            and distance_measure not in self.ALL_DISTANCE_TYPES
        ):
            raise ValueError(
        ...
        self.distance_measure = (
            ...
            else self.DEFAULT_DISTANCE_TYPE
        )
        ...
        self.should_update_template = (
            True if should_update_template is None else should_update_template
        )
        self.should_update_activation = (
            True if should_update_activation is None else should_update_activation
        )
    ...
    def distance(self):
        ...
        """
        return (
            self._euclidean_distance() if self._do_euclidean else self._kl_divergence()
        )
    ...
    def reconstructed_matrix(self):
        ...
        if reconstructed_matrix.shape != self.input_matrix.shape:
            raise Exception(
                "Something went wrong! Reconstructed matrix not the same shape as input_matrix!"
            )
    ...
    def transform(self):
        ...
        if self.should_do_epsilon:
            warnings.warn(
                "User is expected to have set stopping_epsilon prior to using "
    ...
    def _update_activation_euclidean(self):
        ...
        temp_transpose_act = np.dot(
            np.dot(template_transpose, self.template_dictionary), self.activation_matrix
        )
    ...
    def _update_template_euclidean(self):
        ...
        input_matrix_activation_transpose = np.dot(
            self.input_matrix, activation_transpose
        )
        temp_act = np.dot(
            np.dot(self.template_dictionary, self.activation_matrix),
    ...
    def _update_activation_kl_divergence(self):
        ...
        for indices, val in np.ndenumerate(self.activation_matrix):
            (a, mu) = indices
            result = sum(
                (self.template_dictionary[i][a] * self.input_matrix[i][mu]) / dot[i][mu]
                for i in list(range(self.template_dictionary.shape[0]))
            )
            result /= sum(
                self.template_dictionary[k][a]
                for k in list(range(self.template_dictionary.shape[0]))
            )
            result *= self.activation_matrix[indices]
            activation_copy[indices] = result
    ...
    def _update_template_kl_divergence(self):
        ...
        for indices, val in np.ndenumerate(self.template_dictionary):
            (i, a) = indices
            result = sum(
                (self.activation_matrix[a][mu] * self.input_matrix[i][mu]) / dot[i][mu]
                for mu in list(range(self.activation_matrix.shape[1]))
            )
            result /= sum(
                self.activation_matrix[a][nu]
                for nu in list(range(self.activation_matrix.shape[1]))
            )
            result *= self.template_dictionary[indices]
            template_copy[indices] = result
    ...
    def _kl_divergence(self):
        ...
        return sum(
            (
                self.input_matrix[index] * math.log(self.input_matrix[index] / val, 10)
                + self.input_matrix[index]
                - val
            )
            for index, val in np.ndenumerate(self.reconstructed_matrix)
        )
    ...
    def plot(
        ...
        show_divider_lines = (
            show_divider_lines
            if show_divider_lines is not None
        ...
        if matrix_to_dB:
            matrix = librosa.logamplitude(
                np.abs(self.input_matrix) ** 2, ref_power=np.max
            )
            temp_dict = librosa.logamplitude(
                np.abs(self.template_dictionary) ** 2, ref_power=np.max
            )
            activations = librosa.logamplitude(
                np.abs(self.activation_matrix) ** 2, ref_power=np.max
            )
            reconstructed_matrix = librosa.logamplitude(
                np.abs(self.reconstructed_matrix) ** 2, ref_power=np.max
            )

        # Plot the Input Matrix (ax1) and Reconstructed Matrix (ax4)
        matrix_attributes = [
            {"ax": ax1, "mat": matrix, "title": title},
        ...
        if show_divider_lines:
            ax2.set_xticks(np.arange(-0.5, self.num_components, 1), minor=True)
            ax2.set_yticks(np.arange(-0.5, y_len, 1), minor=True)
        ...
        # Minor ticks
        if show_divider_lines:
            ax3.set_yticks(np.arange(-0.5, self.num_components, 1), minor=True)
