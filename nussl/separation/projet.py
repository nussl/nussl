...
class Projet(separation_base.SeparationBase):
    ...
    def create_projections(self):
        ...
        panning_sources_matrix = np.abs(np.random.randn(self.num_panning_directions,
                                                        self.num_sources)).astype(self.matrix_datatype) + 1
        ...
        num_cutoff = 15
        num_fixed = max(self.num_projections - num_cutoff, 0)
        num_random = min(self.num_projections - num_cutoff, num_cutoff)

        projection_matrix = np.concatenate((utils.complex_randn((num_random, num_channels)),
                                            self.orthogonal_matrix(self.multichannel_grid(num_channels, num_fixed))))
        ...
        return K, C, panning_sources_matrix, projection_matrix, projections
    ...
    def run(self):
        ...
        P = np.abs(np.random.randn(num_freq_bins * num_time_bins, self.num_sources)).astype(self.matrix_datatype) + 1

        K, C, panning_sources_matrix, projection_matrix, _ = self.create_projections()
        ...
        # main iterations
        progress_bar = tqdm.trange(self.num_iterations)
        ...
        self.panning_sources_matrix = panning_sources_matrix
