...
class Projet(separation_base.SeparationBase):
    ...
    def __init__(
        ...
        num_sources,
        num_iterations=200,
        num_panning_directions=41,
        num_projections=15,
        matrix_datatype="float32",
        panning_profiles=30,
        verbose=False,
    ...
    def run(self):
        ...
        if self.verbose:
            print("Initializing panning matrix...")
        # initialize PSD and panning to random
        P = (
            np.abs(np.random.randn(num_freq_bins * num_time_bins, num_sources)).astype(
                self.matrix_datatype
            )
            + 1
        )
        ...
        panning_sources_matrix = (
            np.abs(np.random.randn(self.num_panning_directions, num_sources)).astype(
                self.matrix_datatype
            )
            + 1
        )

        chan_pan_diff = utils.complex_randn(
            (num_channels, self.num_panning_directions - self.panning_profiles)
        )
        chan_per_panning_profiles = self.multichannel_grid(
            num_channels, self.panning_profiles
        )

        if self.verbose:
            print("Computing initial panning profiles...")

        # compute panning profiles
        panning_matrix = np.concatenate(
            (chan_pan_diff, chan_per_panning_profiles), axis=1
        )
        panning_matrix /= np.sqrt(np.sum(np.abs(panning_matrix) ** 2, axis=0))[
            None, ...
        ]

        if self.verbose:
            print("Computing initial projection matrices...")

        # compute projection matrix
        projection_matrix = np.concatenate(
            (
                utils.complex_randn((max(self.num_projections - 5, 0), num_channels)),
                self.orthogonal_matrix(
                    self.multichannel_grid(num_channels, min(self.num_projections, 5))
                ),
            )
        )
        projection_matrix /= np.sqrt(np.sum(np.abs(projection_matrix) ** 2, axis=1))[
            ..., None
        ]
        ...
        if self.verbose:
            print(
                "Computing projections and storing spectrograms and squared spectrograms."
            )
        ...
        for iteration in range(self.num_iterations):
            ...
            P *= np.dot(1.0 / (sigma + eps), np.dot(K, panning_sources_matrix)) / (
                np.dot(
                    3 * sigma / (sigma ** 2 + V2 + eps),
                    np.dot(K, panning_sources_matrix),
                )
            )
            ...
            if self.verbose:
                print("\tUpdating panning sources matrix")
            # updating Q
            panning_sources_matrix *= np.dot(
                K.T, np.dot(np.divide(1.0, sigma + eps), P)
            ) / np.dot(K.T, np.dot(np.divide(3 * sigma, (sigma ** 2 + V2.T + eps)), P))

        if self.verbose:
            print("Completing final separation")
        # final separation
        recompose_matrix = np.linalg.pinv(projection_matrix)  # IxM

        sigma = np.dot(P, np.dot(panning_sources_matrix.T, K.T))
        C = np.dot(
            np.reshape(self.stft, (num_freq_bins * num_time_bins, num_channels)),
            projection_matrix.T,
        )
        ...
        if self.verbose:
            print("Making AudioSignal objects")
        for j in range(num_sources):
            ...
            source_stft = np.reshape(
                source_stft, (num_freq_bins, num_time_bins, num_channels)
            )
            source = AudioSignal(
                stft=source_stft, sample_rate=self.audio_signal.sample_rate
            )
            source.istft(
