...
class Repet(mask_separation_base.MaskSeparationBase):
    ...
    def __init__(
        ...
        if period is None:
            ...
            self.max_period = (
                min(8, self.audio_signal.signal_duration / 3)
    ...
    def get_beat_spectrum(self, recompute_stft=False):
        ...
        self.beat_spectrum = self.compute_beat_spectrum(
            np.mean(
                np.square(self.magnitude_spectrogram), axis=self.audio_signal._STFT_CHAN
            ).T
        )
    ...
    def _calculate_repeating_period(self):
        ...
        if self.use_find_period_complex:
            self.repeating_period = self.find_repeating_period_complex(
                self.beat_spectrum
            )
        else:
            ...
            self.repeating_period = self.find_repeating_period_simple(
                self.beat_spectrum, self.min_period, self.max_period
            )
    ...
    def compute_beat_spectrum(power_spectrogram):
        ...
        power_spectrogram = np.vstack(
            [power_spectrogram, np.zeros_like(power_spectrogram)]
        )
        fft_power_spec = scifft.fft(power_spectrogram, axis=0)
        abs_fft = np.abs(fft_power_spec) ** 2
        autocorrelation_rows = np.real(
            scifft.ifft(abs_fft, axis=0)[:freq_bins, :]
        )  # ifft over columns
    ...
    def find_repeating_period_complex(beat_spectrum):
        ...
        for i in range(0, len(beat_spectrum) - 1):
            auto_cosine[i] = 1 - scipy.spatial.distance.cosine(
                beat_spectrum[0 : len(beat_spectrum) - i],
    ...
    def _compute_repeating_mask(self, magnitude_spectrogram_channel):
        ...
        median_mask = np.reshape(
            ...
            (n_repetitions * period, freq_bins),
        ).T
    ...
    def update_periods(self):
        ...
        if self._is_period_converted_to_hops:
            self.period = (
                self._update_period(self.period) if self.period is not None else None
            )
            self.min_period = (
                self._update_period(self.min_period)
                if self.min_period is not None
                else None
            )
            self.max_period = (
                self._update_period(self.max_period)
    ...
    def plot(self, **kwargs):
        ...
        time_vect = np.linspace(
            0.0, self.audio_signal.signal_duration, num=len(beat_spec)
        )
        plt.plot(time_vect, beat_spec)

        if self.repeating_period is not None and show_repeating_period:
            stft_vector = np.linspace(
                0.0, self.audio_signal.signal_duration, self.audio_signal.stft_length
            )
            rep = stft_vector[self.repeating_period]
            plt.plot(
                (rep, rep), (0, np.max(beat_spec)), "g--", label="Repeating period"
            )
