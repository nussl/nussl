...
class Trainer:
    def __init__(
        ...
        self.dataloaders = {"training": self.create_dataloader(train_data)}
        if validation_data:
            self.dataloaders["validation"] = self.create_dataloader(validation_data)
    ...
    def create_dataloader(self, dataset):
        ...
        input_keys = [
            [connection[0]] + connection[1] for connection in self.module.connections
        ]
        ...
        return DataLoader(
            ...
            sampler=Samplers[self.options["sample_strategy"].upper()].value(dataset),
            pin_memory=True,
        )
    ...
    def save(self, is_best: bool, path: str = "") -> str:
        ...
        model_path = os.path.join(
            path if path else self.checkpoint_folder, f"{prefix}.model.pth"
        )
        dataset_options = self.dataloaders["training"].dataset.options
        metadata = {
            key: val
            for key, val in dataset_options.items()
            if key in ["n_fft", "hop_length", "format", "sample_rate", "num_channels"]
        }
