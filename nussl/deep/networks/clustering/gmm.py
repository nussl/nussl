...
class GMM(nn.Module):
    def __init__(
        self, n_clusters, n_iterations=5, covariance_type="diag", covariance_init=1.0
    ):
        """
    ...
    def initialize_parameters(self, data):
        ...
        sampled = (
            data.new(np.arange(0, data.shape[0]))
            .unsqueeze(1)
            .expand(-1, sampled.shape[1])
            * data.shape[1]
            + sampled
        )
        sampled = sampled.long()
        means = torch.index_select(
            data.view(-1, data.shape[-1]), 0, sampled.view(-1)
        ).view(data.shape[0], sampled.shape[-1], -1)

        var = data.new(data.shape[0], self.n_clusters, data.shape[-1]).fill_(
            self.covariance_init
        )
        pi = data.new(data.shape[0], self.n_clusters).fill_(1.0 / self.n_clusters)

        return {"means": means, "variance": var, "prior": pi}
    ...
    def update_parameters(self, posteriors, data, weights):
        ...
        if "tied" in self.covariance_type:
            updated_var = torch.sum(updated_var, dim=1, keepdim=True).expand(
                -1, updated_var.shape[1], -1
            )
        ...
        if "spherical" in self.covariance_type:
            updated_var = torch.mean(updated_var, dim=-1, keepdim=True).expand(
                -1, -1, updated_var.shape[-1]
            )
    ...
    def update_posteriors(self, likelihoods):
        ...
        likelihoods_sum = max_value + torch.log(
            (likelihoods - max_value).exp().sum(dim=1, keepdim=True)
        )
    ...
    def update_likelihoods(self, data, means, var, pi):
        ...
        distance = -0.5 * torch.bmm(distance, inv_covariance)
        ...
        coeff = -0.5 * torch.log(2 * np.pi * var).sum(dim=-1)
        prior = torch.log(pi)
    ...
    def forward(self, data, parameters=None, weights=1.0):
        ...
        posteriors, likelihoods = self.update_posteriors(likelihoods)
        return {
            "likelihoods": likelihoods.permute(0, 2, 1),
