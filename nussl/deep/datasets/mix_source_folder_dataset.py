...
class MixSourceFolder(BaseDataset):
    def __init__(self, folder, options=None):
        ...
        self.channels_in_mix = (
            mix.shape[0] if mix.shape[0] < 8 else int(mix.shape[0] / 2)
        )

    def get_files(self, folder):
        files = [x for x in os.listdir(os.path.join(folder, "mix")) if ".wav" in x]
        files = sorted(
            [x for x in os.listdir(os.path.join(folder, "mix")) if ".wav" in x]
        )

        self.speaker_folders = sorted(
            [x for x in os.listdir(folder) if "s" in x and x != "scaling.mat"]
        )
