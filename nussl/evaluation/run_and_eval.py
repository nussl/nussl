...
def run_and_eval_precision_recall_fscore(separation_object, separation_kwargs,
                                         mixture_list, true_sources_list_of_lists,
    ...
    Args:
        separation_object: 
        separation_kwargs: 
        mixture_list: (list) List of AudioSignal objects that contain mixtures
    ...
    """
    mixture_list = nussl.utils._verify_audio_signal_list_lax(mixture_list)
    if not issubclass(separation_object, nussl.separation.MaskSeparationBase):
        raise ValueError('Expected a MaskSeparationBase derived class!')
    ...
    for i, mixture in enumerate(mixture_list):
        ...
        prf = PrecisionRecallFScore(true_sources_mask_list=ideal_mask_list, estimated_sources_mask_list=est_mask_list)
