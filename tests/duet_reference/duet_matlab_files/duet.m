...
a = abs(R21);
% relative attenuation between the two mixtures
...
p = 1; q = 0;
% powers  used  to  weight  histogram
...
alpha_ind = round(1+(abins-1)*(alphavec+maxa)/(2*maxa));
delta_ind = round(1+(dbins-1)*(deltavec+maxd)/(2*maxd));
...
est = zeros(numsources, length(x1));
% demixtures
for i = 1:numsources
    mask = (bestind == i);
    ...
    soundsc(est(i, :)+0.05*x1, fs);  pause ;
    % play  demixture
end