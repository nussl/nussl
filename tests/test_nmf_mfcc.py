...
class NMFMFCCUnitTests(unittest.TestCase):
    ...
    def test_piano_and_synth(self):
        ...
        nmf_mfcc = nussl.NMF_MFCC(signal, num_sources=2, num_templates=6, distance_measure="euclidean",
                                  num_iterations=10, random_seed=0, mono=False)
        # and run
        nmf_mfcc.run()
        sources = nmf_mfcc.make_audio_signals()
        for i, source in enumerate(sources):
    ...
    def test_piano_and_synth_stereo(self):
        ...
        # Set up NMMF MFCC
        nmf_mfcc = nussl.NMF_MFCC(signal, num_sources=2, num_templates=6, distance_measure="euclidean",
                                  num_iterations=20, random_seed=0, mono=False)
        # and run
        nmf_mfcc.run()
        sources = nmf_mfcc.make_audio_signals()
        for i, source in enumerate(sources):
    ...
    def benchmark_nmf_mfcc(self):
        ...
        benchmark_activation_matrix = np.load(os.path.join('nmf_mfcc_reference', 'test', 'benchmark_activation_matrix.npy'))
        benchmark_templates_matrix = np.load(os.path.join('nmf_mfcc_reference', 'test', 'benchmark_templates_matrix.npy'))
