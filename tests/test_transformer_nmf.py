...
class TransformerNMFUnitTests(unittest.TestCase):
    ...
    def test_small_mixture_matrix(self):
        ...
        for n in range(4, 20, 4):
            ...
            b = np.add(np.multiply(2.0, a), 3.0)
            ...
            self.calculate_nmf_error(
                mixture, self.n_bases, dist_type, self.n_iters, self.n_attempts, n
            )
    ...
    def test_random_matrix(self):
        for n in range(4, 10, 2):
            ...
            self.calculate_nmf_error(
                matrix, self.n_bases, distance_type, self.n_iters, self.n_attempts, n
            )
    ...
    def calculate_nmf_error(
        self, mixture, n_bases, dist_type, iterations, attempts, seed
    ):
        ...
        for i in range(attempts):
            # Set up nussl NMF
            nussl_nmf = nussl.TransformerNMF(
                mixture,
                n_bases,
            ...
            nussl_est = np.dot(
                nussl_nmf.template_dictionary, nussl_nmf.activation_matrix
            )
            ...
            if (
                max_nussl_error > max_nimfa_error
