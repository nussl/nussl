...
class IdealMaskTestCase(unittest.TestCase):
    def setUp(self):
        ...
        self.premade_mixture = nussl.AudioSignal(
            premade_mixture_path, duration=dur, offset=offset
        )
    ...
    def test_run_premade(self):
        ...
        for mask_class in mask_types:
            ideal_mask = nussl.IdealMask(
                self.premade_mixture,
                sources_list=[self.vocals, self.drums],
            ...
            for i, src in enumerate(masked_sources):
                src.write_audio_to_file(
                    os.path.join("..", "output", "mixture", f"premade_src{i}.wav")
                )
            residual.write_audio_to_file(
                os.path.join("..", "output", "mixture", "premade_residual.wav")
            )

            reconstructed = (
                masked_sources[0].audio_data
                + masked_sources[1].audio_data
                + residual.audio_data
            )
    ...
    def test_run_new_mixture(self):
        ...
        for mask_class in mask_types:
            assert np.all(
                np.equal(
                    self.new_mixture.audio_data,
                    self.vocals.audio_data + self.drums.audio_data,
                )
            )
            ...
            if mask_class == nussl.separation.SoftMask:
                for i, src in enumerate(masked_sources):
                    src.write_audio_to_file(
                        os.path.join("..", "output", "mixture", "new_src{i}.wav")
                    )
                residual.write_audio_to_file(
                    os.path.join("..", "output", "mixture", "new_residual.wav")
                )
