...
class DuetUnitTests(BenchmarkTestBase):
    @classmethod
    def setUpClass(cls):
        cls.dev1_female3 = nussl.efz_utils.download_audio_file(
            "dev1_female3_inst_mix.wav"
        )
        cls.dev1_wdrums = nussl.efz_utils.download_audio_file(
    ...
    def test_make_audio_signals(self):
        ...
        final_signals_path = os.path.join(
            "duet_reference", "duet_benchmarks", "benchmark_final_signals.npy"
        )
        ...
        assert all(
            np.array_equal(
                benchmark_final_signals[i].audio_data, final_signals[i].audio_data
            )
            for i in list(range(len(final_signals)))
        )

        # Check to see if AudioSignal's API changed; do we need to refreeze?
        assert all(
            benchmark_final_signals[i] == final_signals[i]
            for i in list(range(len(final_signals)))
        )
