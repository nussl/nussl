...
def main():
    ...
    if DEBUG:
        ...
        output_folder = os.path.join(
            "tests", "nmf_mfcc_reference", "nmf_mfcc_benchmark_files"
        )

    nmf_mfcc.run()
    np.save(
        os.path.join(output_folder, "benchmark_labeled_templates"),
    ...
    json.dump(
        metadata,
        open(os.path.join(output_folder, "nmf_mfcc_benchmark_metadata.json"), "w"),
    )
